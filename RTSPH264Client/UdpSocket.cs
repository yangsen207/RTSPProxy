...
namespace Solace.DotNet.Rtsp {
	public class UDPSocket {
		...
		/// </summary>
		public event EventHandler<Solace.DotNet.Rtsp.RtspChunkEventArgs> DataReceived;

		/// <summary>
		/// Raises the <see cref="E:DataReceived"/> event.
		/// </summary>
		/// <param name="rtspChunkEventArgs">The <see cref="Solace.DotNet.Rtsp.RtspChunkEventArgs"/> instance containing the event data.</param>
		protected void OnDataReceived (Solace.DotNet.Rtsp.RtspChunkEventArgs rtspChunkEventArgs)
		{
		...
		void DoWorkerJob(UdpClient socket, int dataPort)
		{
			...
			try {
				...
				while (true) {
					...
					Solace.DotNet.Rtsp.Messages.RtspChunk currentMessage = new Solace.DotNet.Rtsp.Messages.RtspData ();
					// aMessage.SourcePort = ??
					currentMessage.Data = frame;
					((Solace.DotNet.Rtsp.Messages.RtspData)currentMessage).Channel = dataPort;

					OnDataReceived (new Solace.DotNet.Rtsp.RtspChunkEventArgs (currentMessage));
